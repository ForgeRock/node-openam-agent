...
OpenAMClient.prototype.getProfile = function (userId, realm, agentSessionId, cookieName) {
    var headers = {};
    headers[cookieName] = agentSessionId;

    realm = realm ? realm.replace(/\/$/, '') : ''
    ;
    if (realm.length && realm.indexOf('/') !== 0)
        realm = '/' + realm;

    return request.get(this.serverUrl + '/json' + realm + '/users/' + userId, {
        headers: headers,
        json: true
    });
};
