...
function PolicyAgent(options) {
    ...
    options.api_realm = buildRealmPath(options.realm);
    ...
    shutdownHandler.on('exit', function (e) {
        if (self.agentSession.sessionId) {
            ...
            self.openAMClient
                .logout(self.agentSession.sessionId, self.serverInfo.cookieName, self.options.api_realm)
...
/** 
 * Builds realm path used by /json endpoints: for details see https://backstage.forgerock.com/docs/am/6/dev-guide/#sec-rest-realms
 */

 function buildRealmPath(realm) {
    var result = '';
        var subrealms = realm.split('/');
        subrealms[0] = 'root';
        subrealms.forEach(function(r) {
            if (r.length > 0) 
                result += '/realms/' + r;
        })
...
PolicyAgent.prototype.authenticateAgent = function () {
    ...
    return self.agentSession = self.serverInfo
        .then(function () {
            return self.openAMClient.authenticate(self.options.username, self.options.password, self.options.api_realm);
        })
        .then(function (res) {
    ...
    return this.sessionCache.get(sessionId)
        ...
        .catch(function (err) {
            ...
            return self.agentSession.then(function () {
                return self.openAMClient.getProfile(userId, self.options.api_realm, sessionId, self.serverInfo.cookieName).then(function (profile) {
...
PolicyAgent.prototype.getPolicyDecision = function (params) {
    ...
    return this.reRequest(function () {
        return self.openAMClient.getPolicyDecision(params, self.options.api_realm, self.agentSession.sessionId, self.serverInfo.cookieName);
    }, 5, 'getPolicyDecision');
};
