 ...
 * @param {boolean} [options.letClientHandleErrors] when set to true if an error arises then next() will be called
 * allowing the client application to handle the errors. When set to false then a text/html response is made on behalf
 * of the client. Default is true.
...
function PolicyAgent(options) {
    ...
    /**
     * Controls whether the agent sends html response on behalf of the client or lets the client handle the error
     * @type {boolean}
     */
    this.letClientHandleErrors = options.letClientHandleErrors || false;
    ...
    shutdownHandler.on('exit', function (e) {
        if (self.agentSession.sessionId) {
            ...
            self.openAMClient
                .logout(self.agentSession.sessionId, self.serverInfo.cookieName, self.options.realm)
                .finally(process.exit);
        }

        // close the cache
        self.sessionCache.quit();
    });
}
 ...
 *
 * @param {http~ServerResponse} res Express Response
 ...
 *
 * @param {http~IncomingMessage} req Express Request
...
PolicyAgent.prototype.getSessionIdFromRequest = function (req) {
    ...
    return this.serverInfo.then(function (serverInfo) {
        ...
        if (sessionId) {
            ...
            self.logger.info('PolicyAgent: missing session ID in request cookie %s', sessionId, serverInfo.cookieName);
        }
    ...
    return this.sessionCache.get(sessionId)
        ...
        .catch(function (err) {
            ...
            return self.agentSession.then(function () {
                return self.openAMClient.getProfile(userId, self.options.realm, sessionId, self.serverInfo.cookieName).then(function (profile) {
...
PolicyAgent.prototype.getPolicyDecision = function (params) {
    ...
    return this.reRequest(function () {
        return self.openAMClient.getPolicyDecision(params, self.agentSession.sessionId, self.serverInfo.cookieName, self.options.realm);
    }, 5, 'getPolicyDecision');
};
...
PolicyAgent.prototype.shield = function (shield) {
    ...
    return function (req, res, next) {
        shield.evaluate(req, res, self)
            .then(function (session) {
                req.session = util._extend(req.session || {}, session);
                next();
            })
            .catch(function (err) {
                self.logger.info('PolicyAgent#shield: evaluation error (%s)', err.message);
                // handle response if unhandled
                if(self.letClientHandleErrors){
                    next(err);
                } else {
                    if (!res.headersSent) {
                        httpUtils.sendResponse(res, err.statusCode, self.errorTemplate({
...
PolicyAgent.prototype.getLoginUrl = function (req) {
    return this.openAMClient.getLoginUrl(httpUtils.baseUrl(req) + req.url, this.options.realm);
};
...
PolicyAgent.prototype.getCDSSOUrl = function (req) {
    var target = httpUtils.baseUrl(req) + this.paths.cdsso + '?goto=' +
